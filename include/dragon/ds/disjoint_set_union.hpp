 ...
 * of trees, with union by rank and path compression heuristics to improve
 * runtime complexity.
 *
 * @param ValueT type of data
 */
template <typename ValueT> class DisjointSetUnion {
public:
...
template <typename ValueT>
typename DisjointSetUnion<ValueT>::SizeType
DisjointSetUnion<ValueT>::find_representative(SizeType u_i) {
  if (m_parent[u_i] != u_i) {
...
template <typename ValueT>
bool DisjointSetUnion<ValueT>::in_same_set(const ValueType& u,
                                           const ValueType& v) {
  return find_representative(m_identifier[u]) ==
         find_representative(m_identifier[v]);
}
